...
The first chunk of code below loads necessary libraries and defines some objects which will be used throughout the analysis. The DSN will specified will depend on how a particular reader connects to the FamLink Data Warehouse. Individual API keys for the US Census can be found here: [https://api.census.gov/data/key_signup.html](https://api.census.gov/data/key_signup.html).
...
```{r zcta_cross_walk, message=FALSE, warning=FALSE}
...
```{r zcta_placement_counts, message=FALSE, warning=FALSE}
...
The next section generates population estimates from the US Census. We specifically make use of the 5-year ACS estimates. As of the date of this writing, the most recent ACS data available are 2016. 
...
After cleaning up those aggregations which come from low population ZCTAs, we finally calculate an expected removal rate.
...
We next calculate the centroid of each ZCTA in the boundary file. We then find near neighbors to each ZCTA centroid. We specifically look for any ZCTA that is within 20 km of another ZCTA. For each ZCTA, the vector of inverse distances (i.e. 1/distance) for each neighbor is multiplied against the placement rate for the neighbor. All values (along with the value for the ZCTA in question) are then averaged together. The result, for each ZCTA is a weighted average of local placement rates.
...
The interactive map produced by this code is available at (my link).
